...
VCF_UNFILT="/scratch.global/liux1299/temp_mut8_and_hybrid_barley/mut8_and_hybrid_barley_snps_polymorphic.vcf.gz"
FILT1_VCF="/panfs/jay/groups/9/morrellp/shared/Datasets/Alignments/mut8_and_hybrid_barley/Filtered/mut8_and_hybrid_barley_snps_biallelic.vcf.gz"
FILT2_VCF="/panfs/jay/groups/9/morrellp/shared/Datasets/Alignments/mut8_and_hybrid_barley/Filtered/mut8_and_hybrid_barley_snps_biallelic.noRepeatOverlap.noRefNs.vcf.gz"
...
VCF_OUT_PREFIX="mut8_and_hybrid_barley_snps_polymorphic"
OUT_DIR="/panfs/jay/groups/9/morrellp/shared/Datasets/Alignments/mut8_and_hybrid_barley/Filtered/scikit_allel_files"
# Select chr1H only
CHROM_PREFIX="chr1H"
...
bcftools annotate -a ${OUT_DIR}/ann_known-${VCF_OUT_PREFIX}.txt.gz -h ${OUT_DIR}/ann_known-${VCF_OUT_PREFIX}.hdr -c CHROM,POS,REF,ALT,-,VAR_KNOWN ${VCF_UNFILT} > ${OUT_DIR}/ann_known-${VCF_OUT_PREFIX}.vcf

# Add filtered TAG for filtered vs retained_filt1 vs retained_filt2 to INFO field
bcftools annotate -a ${OUT_DIR}/ann_filt-${VCF_OUT_PREFIX}.txt.gz -h ${OUT_DIR}/ann_filt-${VCF_OUT_PREFIX}.hdr -c CHROM,POS,REF,ALT,-,VAR_FILT ${OUT_DIR}/ann_known-${VCF_OUT_PREFIX}.vcf > ${OUT_DIR}/ann_known_and_filt-${VCF_OUT_PREFIX}.vcf

# Cleanup intermediate file with duplicate info
rm ${OUT_DIR}/ann_known-${VCF_OUT_PREFIX}.vcf

# Select chr1H for visual exploration
bcftools view -t "chr1H_part1,chr1H_part2" ${OUT_DIR}/ann_known_and_filt-${VCF_OUT_PREFIX}.vcf > ${OUT_DIR}/ann_known_and_filt-${CHROM_PREFIX}_${VCF_OUT_PREFIX}.vcf

# Pull out known variants that got filtered out
bcftools filter -i 'VAR_KNOWN="known" & VAR_FILT!="retained_filt2"' ${OUT_DIR}/ann_known_and_filt-${VCF_OUT_PREFIX}.vcf > ${OUT_DIR}/only_known-filtered-retained_filt1-${VCF_OUT_PREFIX}.vcf

bcftools filter -i 'VAR_KNOWN="known"' ${OUT_DIR}/ann_known_and_filt-${VCF_OUT_PREFIX}.vcf > ${OUT_DIR}/only_known-${VCF_OUT_PREFIX}.vcf
