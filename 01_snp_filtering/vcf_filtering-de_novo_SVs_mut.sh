...
# Callable ONT INDELs
VCF_ONT="/panfs/jay/groups/9/morrellp/shared/Datasets/Alignments/nanopore_mutated_barley/filtered/mut_ont.geSup3.callable.INDELs.vcf"
...
# None of the larger svs remain after removing overlaps with diffs from ref
...
svplaudit_scored="/panfs/jay/groups/9/morrellp/shared/Projects/Mutant_Barley/samplot-mut_10x/mut_10x_dels.callable.noRefDiffs.private.supports.vcf"

# Rescue SV that got removed during SV-Plaudit scoring but has additional support from ONT
bedtools intersect -wa -a ${OUT_DIR}/${dels_bn}.noRefDiffs.private.vcf.gz -b ${VCF_ONT} | uniq > ${OUT_DIR}/tmp_${dels_bn}_overlap_ONT.noRefDiffs.private.vcf
...
mv ${OUT_DIR}/tmp_${dels_bn}_overlap_ONT.noRefDiffs.private.vcf \
    ${OUT_DIR}/*_merged.callable.noRefDiffs.vcf \
